...
| `src/auth/dpop-handler.ts` | composable token-minting orchestration with injected atomic JTI consumer (testable without DOs). **Not on the live path** — the `/token` route in `worker.ts` implements this inline against `authority.mintDPoPTokenOnce`; `handleToken` is referenced only by its own tests (see `notme-e1700d`)                                                                                                                                              | `handleToken`, `buildJwksResponse`                                                                                                                       |
...
cd worker
npm ci
npx vitest run                              # unit (406): src/__tests__/ + ../packages/dpop/__tests__ (vault retired — see docs/design/012-vault-retirement.md)
