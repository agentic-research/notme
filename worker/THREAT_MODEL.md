...
| challenge expiry | use a stale challenge | challenges older than 5 min rejected at verify time; `passkey_challenges` is also swept of rows older than 1 hour on every options call so the table doesn't grow unbounded from abandoned flows (notme-ae81c9). | `passkey.challenge.expiry`, `passkey.challenge.sweeper` |
...
| runaway alarm bill | unguarded `setAlarm()` calls accidentally compound (e.g. an `onStart()`-style hook calling setAlarm without a getAlarm check), or many DO instances each running their own 4-min alarm loop, scaling DO reads/writes catastrophically (the r/CloudFlare cautionary tale: 20T DO reads on a single invoice) | **FIXED** — `scheduleNextRefresh()` always checks `getAlarm()` before `setAlarm()`. The `alarm()` re-arm path uses the same defense-in-depth check (notme-5c5481). All call sites for setAlarm are idempotent: scheduling a second alarm before the first fires is a no-op. Single DO instance per deploy (`idFromName("default")`); no per-request DO spawning. | `signing.alarm.idempotent-reschedule` (pending notme-c38bb6 vitest pool config) |
| failure-storm alarm loop | `generateBundle()` throws repeatedly (DO storage corruption, key import failure, KV unavailable, signing-key state desync); previous `try/catch` swallowed errors and re-armed unconditionally → alarm fires every 4 min forever, each fire doing real DO storage work | **FIXED** — circuit breaker at MAX_CONSECUTIVE_ALARM_FAILURES = 5 strikes (~20min of failures before kill switch fires). After trip, `alarm()` returns without re-arming. Manual recovery via `resetAlarmHealth()` RPC. Failure count + last_outcome persisted to `alarm_health` SQLite table (notme-5ca7c6). | `signing.alarm.circuit-breaker` (pending notme-c38bb6) |
| invisible runaway escalation | a future code change reintroduces a setAlarm bug, or a CF runtime quirk fires `alarm()` faster than the 4-min cadence; without internal observability we'd notice only on the CF invoice (the r/CloudFlare scenario) | **DETECT** — `alarm_health` table tracks total_fires, first_fire_at, last_fire_at, failure_count, last_outcome. `getAlarmHealth()` RPC computes `firesPerHour / expectedFiresPerHour = driftRatio` along with a `warmupComplete` flag. **driftRatio is meaningful only after warmup** (`total_fires >= 3`, ~12 minutes); during startup the average is too noisy (single fire over short elapsed time → arbitrarily large rate). After warmup, sustained `driftRatio > 1.2` indicates the alarm is firing faster than the 4-min cadence prescribes — runaway smoking gun. Consumers must gate alerts on `warmupComplete === true` (notme-5cea0e). | `signing.alarm.health-counter` (pending notme-c38bb6) |
| preview-deploy alarm fan-out | preview deployments (per-PR or workers_dev) each provision a separate SigningAuthority DO with its own 4-min alarm; 60+ previews × 15 fires/hour × N years = the redditor's exact escalation curve | **OPEN** — currently `workers_dev = false` and no preview-deploy infra exists, so this surface is dormant. Tracked as notme-5d358a (P3, trigger-based). Pre-flight checklist in the bead: pick policy (no-preview-DO / bounded lifetime / disable-alarms-in-preview), add CI gate verifying fire count, set CF spending cap if available. | (none yet — defer until preview infra lands) |
