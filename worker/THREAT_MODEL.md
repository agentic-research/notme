...
| invisible runaway escalation | a future code change reintroduces a setAlarm bug, or a CF runtime quirk fires `alarm()` faster than the 4-min cadence; without internal observability we'd notice only on the CF invoice (the r/CloudFlare scenario) | **DETECT** — `alarm_health` table tracks total_fires, first_fire_at, last_fire_at, failure_count, last_outcome. New `getAlarmHealth()` RPC computes `firesPerHour / expectedFiresPerHour = driftRatio`. driftRatio > 1 indicates the alarm is firing faster than the 4-min cadence prescribes — runaway smoking gun, queryable from any auth-gated caller (notme-5cea0e). | `signing.alarm.health-counter` (pending notme-c38bb6) |
