...
import VECTORS from "./vectors/receipt-canonical-bytes.json";
...
describe("cross-implementation contract — RECEIPTS.md §7 vectors", () => {
  // The spec-mandated receipt-canonical-bytes.json, generated by encoding each
  // `input` with BOTH cloister's encoder and notme's and requiring agreement.
  // Cloister pins the identical file, so a change to the commitment shape
  // fails on the side that made it rather than at integration.
  //
  // Before the hand-rolled §4.2 encoder landed, notme rejected these exact
  // bytes with FIELD_NOT_UINT — cbor-x decodes a shortest-form uint64 to
  // BigInt — which was 100% of cloister's real traffic.
  //
  // If these fail, the two sides have diverged on the signing input and every
  // receipt notme signs is unverifiable. Do not loosen the validator; find out
  // which side moved.
  const vectors = VECTORS.vectors as Array<{
    name: string;
    input: Record<string, unknown>;
    expected_canonical_cbor_hex: string;
  }>;

  it("pins all §7 landmine cases", () => {
    expect(vectors.length).toBeGreaterThanOrEqual(12);
  });

  for (const v of vectors) {
    it(`accepts and re-encodes identically: ${v.name}`, () => {
      const bytes = Uint8Array.from(
        v.expected_canonical_cbor_hex.match(/../g)!.map((h) => parseInt(h, 16)),
      );
      const facts = {
        actorFp: Uint8Array.from(
          (v.input.actor_fp_hex as string)
            .match(/../g)!
            .map((h) => parseInt(h, 16)),
        ),
        epoch: v.input.epoch as number,
        nowMs: v.input.timestamp_ms as number,
      };
      const out = validateCommitment(bytes, facts);
      expect(Array.from(out)).toEqual(Array.from(bytes));
      // §3.1: floats are forbidden outright.
      expect(Array.from(out)).not.toContain(0xfb);
    });
  }
});
